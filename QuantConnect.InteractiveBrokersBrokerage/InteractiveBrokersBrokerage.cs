...
namespace QuantConnect.Brokerages.InteractiveBrokers
{
    ...
    public sealed class InteractiveBrokersBrokerage : Brokerage, IDataQueueHandler, IDataQueueUniverseProvider
    {
        ...
        public override void Connect()
        {
            ...
            while (!_isDisposeCalled)
            {
                try
                {
                    ...
                    _client.ClientSocket.reqAccountSummary(GetNextId(), "All", "AccountType");  // see https://ibkrcampus.com/ibkr-api-page/twsapi-doc/#account-summary-tags for valid tags
        ...
        /// <summary>
        /// Submits a tick-by-tick (i.e. tape data) request (a subscription) for a given contract to IB.
        /// See https://ibkrcampus.com/ibkr-api-page/twsapi-doc/#request-tick-data
        /// </summary>
        private void RequestTape(Contract contract, int requestId)
        {
                    ...
                    if (CanSubscribe(symbol))
                    {
                        lock (_sync)
                        {
                            ...
                            if (_subscribedSymbols.TryRemove(symbol, out id))
                            {
                                ...
                                // Client.ClientSocket.cancelTickByTickData(id); // TODO, gotta handle tape data cancellation
        ...
        {
            return DateTime.UtcNow.ConvertFromUtc(GetSymZone(symbol));
        }

        private DateTimeZone GetSymZone(Symbol symbol)
        {
        ...
        {
            return DateTime.UnixEpoch.AddSeconds(unixEpoch).ConvertFromUtc(GetSymZone(symbol));
        }
        ...
        private void HandleTape(object sender, IB.TapeEventArgs e)
        {
            ...
            if (!_subscribedTickers.TryGetValue(e.TickerId, out entry))
            {
                Console.WriteLine("HandleTape() BOOO :(");
                return;
            }
        ...
        public override IEnumerable<BaseData> GetHistory(HistoryRequest request)
        {
            ...
            DateTimeZone exchangeTimeZone = GetSymZone(request.Symbol);
